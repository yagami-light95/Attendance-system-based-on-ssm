...
<body>
				...
				<div class="panel panel-default">
				    <div class="panel-heading">
						<div class="row">
							...
							<form class="form-horizontal" style="margin: 20px 0 10px 0;" role="form" action="<%=request.getContextPath()%>/admin/showReportInfo" id="querySign" method="post">
								<div class="form-group">
										...
										<div class="control-group">
											<div class="controls">
												<div class="input-prepend input-group">
													...
													<input type="text" name="createDate" id="createDate" class="form-control span3 text-center" value="${createDate}" placeholder="请选择查询时间"/>
												</div>
											</div>
										</div>
									</div>
									...
									<button class="btn btn-primary" type="button" onclick="analysis()">考勤统计</button>
								</div>
							</form>
				    ...
				    <div class="panel-footer">
						<c:if test="${pagingVO != null}">
							<nav style="text-align: center">
								<ul class="pagination">
									<li><a href="<%=request.getContextPath()%>/admin/showReportInfo?page=${pagingVO.upPageNo}&createDate=${createDate}&departmentId=${departmentId}">&laquo;上一页</a></li>
									<li class="active"><a href="">${pagingVO.curentPageNo}</a></li>
									<c:if test="${pagingVO.curentPageNo+1 <= pagingVO.totalCount}">
										<li><a href="<%=request.getContextPath()%>/admin/showReportInfo?page=${pagingVO.curentPageNo+1}&createDate=${createDate}&departmentId=${departmentId}">${pagingVO.curentPageNo+1}</a></li>
									</c:if>
									<c:if test="${pagingVO.curentPageNo+2 <= pagingVO.totalCount}">
										<li><a href="<%=request.getContextPath()%>/admin/showReportInfo?page=${pagingVO.curentPageNo+2}&createDate=${createDate}&departmentId=${departmentId}">${pagingVO.curentPageNo+2}</a></li>
									</c:if>
									<c:if test="${pagingVO.curentPageNo+3 <= pagingVO.totalCount}">
										<li><a href="<%=request.getContextPath()%>/admin/showReportInfo?page=${pagingVO.curentPageNo+3}&createDate=${createDate}&departmentId=${departmentId}">${pagingVO.curentPageNo+3}</a></li>
									</c:if>
									<c:if test="${pagingVO.curentPageNo+4 <= pagingVO.totalCount}">
										<li><a href="<%=request.getContextPath()%>/admin/showReportInfo?page=${pagingVO.curentPageNo+4}&createDate=${createDate}&departmentId=${departmentId}">${pagingVO.curentPageNo+4}</a></li>
									</c:if>
									<li><a href="<%=request.getContextPath()%>/admin/showReportInfo?page=${pagingVO.totalCount}&createDate=${createDate}&departmentId=${departmentId}">最后一页&raquo;</a></li>
								</ul>
							</nav>
						</c:if>
				    </div>
	...
	<script type="text/javascript">
        ...
        function analysis() {
          $.ajax({
            url: '<%=request.getContextPath()%>/admin/analysis',
            type:"GET",
            dataType:"json",
            async:false,
